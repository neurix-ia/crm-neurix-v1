...
export default function VendiNovaVendaPage() {
    ...
    const fotoBlob = useRef<File | null>(null);
    ...
    const pararGravacao = useCallback(() => {
        if (mediaRecorder.current && gravando) {
    ...
    const onFoto = (file: File | undefined) => {
        if (!file) return;
        fotoBlob.current = file;
    ...
    const enviar = async () => {
        ...
        if (!fotoBlob.current || numero.replace(/\D/g, "").length < 8 || !temProduto) {
            showToast("Falta produto, foto da placa ou WhatsApp do cliente", true);
            return;
        }
        setSending(true);
        try {
            const payload = {
                ...
                foto_base64: await blobToBase64(fotoBlob.current),
                foto_mime: fotoBlob.current.type || "image/jpeg",
    ...
    const canSend =
        !!fotoPreview && whatsapp.trim().replace(/\D/g, "").length >= 8 && quantidades.italiano + quantidades.integral > 0;
        ...
        <div className="min-h-screen bg-[#1a1410] text-[#f5ece0] flex flex-col font-sans">
            <div className="flex-1 flex flex-col p-5 max-w-lg mx-auto w-full">
                ...
                <p className="text-sm text-[#b8a892] mb-6">
                    Toque pra gravar, converse com o cliente, e feche tirando a foto da placa.
                </p>
                ...
                >
                    ...
                    <div className="text-sm text-[#b8a892]">{recStatus}</div>
                </button>
                ...
                <label className="text-xs text-[#b8a892] mb-1 block">Foto da placa do carro</label>
                <button
                    type="button"
                ...
                />
                <p className="text-xs text-[#b8a892] mb-4">Tirar a foto encerra a gravação automaticamente.</p>
